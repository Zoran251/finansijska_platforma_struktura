...
class SupportChat {
    ...
    createChatUI() {
        ...
        console.log('🔍 Elementi provereni:', {
            input: !!input,
            button: !!button,
            messages: !!messages,
...
// Export za debug
window.debugSupportChat = {    getStatus: () => supportChatInstance?.getStatus(),
    clearMessages: () => supportChatInstance?.clearMessages(),
    exportMessages: () => supportChatInstance?.exportMessages(),
    testResponse: (msg) => supportChatInstance?.testResponse(msg),
    openChat: () => supportChatInstance?.openChat(),
    closeChat: () => supportChatInstance?.closeChat()
};

// Označimo da je SupportChat učitan
window.GoldenBalanceSupportChat = true;
console.log('🎧 SupportChat klasa učitana i spremna!');

// Force override bilo kakvih starih chat sistema
window.openChat = () => {
    console.log('🔄 Pozvan stari openChat(), preusmjeravam na novi sistem');
    if (window.supportChatInstance) {
        window.supportChatInstance.openChat();
    }
};

window.closeChat = () => {
    console.log('🔄 Pozvan stari closeChat(), preusmjeravam na novi sistem');
    if (window.supportChatInstance) {
        window.supportChatInstance.closeChat();
    }
};

// Override bilo kakve stare funkcije
window.addMessage = () => {
    console.log('🔄 Pozvan stari addMessage(), koristim novi sistem');
};

console.log('🎯 NOVA Golden Balance Support Chat implementacija spremna!');

} else {
    console.log('🔄 SupportChat već učitan, preskačem inicijalizaciju');
}
